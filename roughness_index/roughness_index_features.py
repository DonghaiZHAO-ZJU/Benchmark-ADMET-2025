...
representations = ["MorganFP", "rdkit_desc", "mordred_desc", "KPGT_Embedding","GEM_Embedding","Uni-Mol_Embedding","K_BERT_Embedding"]
...
def get_sim_matrix(X=None, scale=False):
    ...
    for i in range(len(X)):
        for j in range(i, len(X)):
            ...
            sim_matrix[i, i] = 1  # Fill diagonal
...
for task in dataset_names:
    ...
    origin_data1 = pd.read_csv(f'./data/origin_data/{task}_random_2024.csv')
    ...
    for representation in representations:
        ...
        if representation in ('MorganFP'):
            data_with_representation = pd.read_csv(f"data/processed_data/{representation}/{task}_random_2024_{representation}.csv")
...
df.to_csv("SARI(Dynamic_Thresholds).csv", index=False)
...
for task in dataset_names:
    ...
    for representation in representations:
        ...
        if representation in ('MorganCount','phaERGFP'):
            ...
            if task=='PAMPA1':
                data_with_representation = pd.read_csv(f"data/processed_data/{representation}/PAMPA_random_2024_{representation}.csv")
...
df.to_csv("ROGI.csv", index=False)
...
df.to_csv("MODI.csv", index=False)