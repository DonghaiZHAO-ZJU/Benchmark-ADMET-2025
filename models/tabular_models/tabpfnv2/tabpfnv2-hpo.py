...
if args.task_type == 'classification':
    ...
    for i in range(5):
        print(f'------time: {i+1}------')    
        if not os.path.exists(f'./TabPFN2_hpo_prediction/{args.fp_name}/{args.data_name}_{i+1}.csv'):
            clf = TunedTabPFNClassifier(n_trials=30, metric='roc_auc', random_state=2024 + i * 10)
            clf.fit(train_x, train_y, valid_x, valid_y)
            train_prediction, valid_prediction, test_prediction = [clf.predict_proba(x)[:, 1] for x in (train_x, valid_x, test_x)]
            prediction = pd.DataFrame({'smiles':test_smiles, 'pred': test_prediction, 'label': test_y})
            os.makedirs(f'./TabPFN2_hpo_prediction/{args.fp_name}/', exist_ok=True)
            prediction.to_csv(f'./TabPFN2_hpo_prediction/{args.fp_name}/{args.data_name}_{i+1}.csv', index=False) 
...
# Regression
    ...
    for i in range(5):
        print(f'------time: {i+1}------')
        if not os.path.exists(f'./TabPFN2_hpo_prediction/{args.fp_name}/{args.data_name}_{i+1}.csv'):
            reg = TunedTabPFNRegressor(n_trials=30, random_state=2024 + i * 10)
            reg.fit(train_x, train_y, valid_x, valid_y)
            train_prediction, valid_prediction, test_prediction = [reg.predict(x) for x in (train_x, valid_x, test_x)]
            prediction = pd.DataFrame({'smiles':test_smiles, 'pred': test_prediction, 'label': test_y})
            os.makedirs(f'./TabPFN2_hpo_prediction/{args.fp_name}/', exist_ok=True)
            prediction.to_csv(f'./TabPFN2_hpo_prediction/{args.fp_name}/{args.data_name}_{i+1}.csv', index=False)
...
# os.makedirs(f'./TabPFN2_hpo_result/{args.fp_name}/', exist_ok=True)
# result_pd.to_csv(f'./TabPFN2_hpo_result/{args.fp_name}/TabPFN_{args.fp_name}_{args.data_name}_all_result.csv', index=False)