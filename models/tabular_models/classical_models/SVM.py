...
space = {'C':hp.choice('C', [0.1, 1, 10, 100]),
         'gamma': hp.choice('gamma', ['scale', 'auto', 1, 0.1, 0.01, 0.001, 0.0001, 0.00001]),
...
if args.task_type=='classification':
    best = fmin(hyperopt_my_xgb_classification, space, algo=tpe.suggest, trials=trials, max_evals=15)
else:
    best = fmin(hyperopt_my_xgb_regression, space, algo=tpe.suggest, trials=trials, max_evals=15)
print(best)

args.C = [0.1, 1, 10, 100][best['C']]
args.gamma = ['scale', 'auto', 1, 0.1, 0.01, 0.001, 0.0001, 0.00001][best['gamma']]
args.kernel = ['rbf', 'sigmoid'][best['kernel']]
...
if args.task_type=='classification':
    ...
    for i in range(5):
        ...
        os.makedirs(f'./SVM_prediction/{args.fp_name}/', exist_ok=True)
        prediction.to_csv(f'./SVM_prediction/{args.fp_name}/SVM_{args.fp_name}_{args.data_name}_{i+1}_test_prediction.csv', index=False) 
...
os.makedirs(f'./SVM_result/{args.fp_name}/', exist_ok=True)
result_pd.to_csv(f'./SVM_result/{args.fp_name}/SVM_{args.fp_name}_{args.data_name}_all_result.csv', index=False)