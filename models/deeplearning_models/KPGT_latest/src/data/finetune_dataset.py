...
class MoleculeDataset(Dataset):
    def __init__(self, root_path, dataset, dataset_type, use_scaler, path_length=5, n_virtual_nodes=2, split_name=None, split=None):
        ...
        elif dataset_type == 'regression' and use_scaler:
            print('use label scaler')
            self.set_mean_and_std()
        else:
            print('no use label scaler')
    ...
    def set_mean_and_std(self, mean=None, std=None):
        ...
        self.std = std
        # if mean is None:
        #     median = torch.from_numpy(np.nanmedian(self.labels.numpy(), axis=0))
        # if std is None:
        #     q1 = torch.from_numpy(np.nanpercentile(self.labels.numpy(), 25, axis=0))
        #     q3 = torch.from_numpy(np.nanpercentile(self.labels.numpy(), 75, axis=0))
        #     iqr = q3 - q1
        # self.mean = median
        # self.std = iqr.float()