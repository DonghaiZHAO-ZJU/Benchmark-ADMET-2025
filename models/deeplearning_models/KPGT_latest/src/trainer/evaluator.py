from sklearn.metrics import roc_auc_score, average_precision_score, mean_absolute_error, r2_score, precision_recall_curve, auc
import pandas as pd
import os
import numpy as np
...
class Evaluator:
    ...
    def _eval_prauc(self, y_true, y_pred):
        '''
            compute PR-AUC averaged across tasks
        '''
        ...
        for i in range(y_true.shape[1]):
            ...
            if np.sum(y_true[:,i] == 1) > 0 and np.sum(y_true[:,i] == 0) > 0:
                # ignore nan values
                is_labeled = y_true[:,i] == y_true[:,i]
                precision, recall, _thresholds = precision_recall_curve(y_true[is_labeled,i], y_pred[is_labeled,i])
                prauc_list.append(auc(recall, precision))

        if len(prauc_list) == 0:
            raise RuntimeError('No positively labeled data available. Cannot compute PR-AUC.')

        return sum(prauc_list)/len(prauc_list)
    ...
    def _eval_acc(self, y_true, y_pred):
        ...
        for i in range(y_true.shape[1]):
            is_labeled = y_true[:,i] == y_true[:,i]
            y_pred = y_pred[is_labeled,i]
            y_pred = torch.sigmoid(torch.from_numpy(y_pred)).tolist()
            y_pred = [1 if x >= 0.5 else 0 for x in y_pred]
            correct = y_true[is_labeled,i] == y_pred
