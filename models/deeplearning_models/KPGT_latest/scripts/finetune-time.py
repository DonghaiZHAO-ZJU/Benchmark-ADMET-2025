...
def finetune(args):
    ...
    best_train, best_val, best_test, best_train1, best_val1, best_test1, best_train2, best_val2, best_test2 = trainer.fit(model, train_loader, val_loader, test_loader, model_path=f'../model/model.pth')
    train_end = time.time()
    train_time = train_end - train_start
    print('train_time: ', train_time)
    print(f"train: {best_train:.4f}, val: {best_val:.4f}, test: {best_test:.4f}", flush=True)
    model.load_state_dict(torch.load(f'../model/model.pth')['model_state_dict'])
    eval_start = time.time()
    end_test_result = trainer.eval(model, test_loader)
    eval_end = time.time()
    eval_time = eval_end - eval_start
    print('eval_time: ', eval_time)
    print('end_test_result:', end_test_result)
    return best_train, best_val, best_test, best_train1, best_val1, best_test1, best_train2, best_val2, best_test2, train_time, eval_time
...
if __name__ == '__main__':
    ...
    for time_id in range(args.times):
        ...
        best_train, best_val, best_test, best_train1, best_val1, best_test1, best_train2, best_val2, best_test2, train_time, eval_time = finetune(args)
        ...
        testing_times.append(eval_time)
