...
def finetune(args):
    ...
    best_train, best_val, best_test, best_train1, best_val1, best_test1, best_train2, best_val2, best_test2 = trainer.fit(model, train_loader, val_loader, test_loader)
...
if __name__ == '__main__':
    ...
    for time_id in range(args.times):
        ...
        evaluation_results.append([best_train, best_val, best_test])
        evaluation_results1.append([best_train1, best_val1, best_test1])
        evaluation_results2.append([best_train2, best_val2, best_test2])
        args.seed += 1
    df = pd.DataFrame(evaluation_results, columns=['train', 'valid', 'test'])
    df = df.iloc[:5].rename(index=lambda x: f"Run {x+1}")
    df.loc['mean'] = df.mean().round(4)
    df.loc['std'] = df.std().round(4)
    os.makedirs(f'../result_new/{args.datasplit_method}/{args.metric[0]}/', exist_ok=True)
    df.to_csv(f'../result_new/{args.datasplit_method}/{args.metric[0]}/{args.dataset}_{args.metric[0]}.csv')

    df1 = pd.DataFrame(evaluation_results1, columns=['train', 'valid', 'test'])
    df1 = df1.iloc[:5].rename(index=lambda x: f"Run {x+1}")
    df1.loc['mean'] = df1.mean().round(4)
    df1.loc['std'] = df1.std().round(4)
    os.makedirs(f'../result_new/{args.datasplit_method}/{args.metric[1]}/', exist_ok=True)
    df1.to_csv(f'../result_new/{args.datasplit_method}/{args.metric[1]}/{args.dataset}_{args.metric[1]}.csv')

    df2 = pd.DataFrame(evaluation_results2, columns=['train', 'valid', 'test'])
    df2 = df2.iloc[:5].rename(index=lambda x: f"Run {x+1}")
    df2.loc['mean'] = df2.mean().round(4)
    df2.loc['std'] = df2.std().round(4)
    os.makedirs(f'../result_new/{args.datasplit_method}/{args.metric[2]}/', exist_ok=True)
    df2.to_csv(f'../result_new/{args.datasplit_method}/{args.metric[2]}/{args.dataset}_{args.metric[2]}.csv')
