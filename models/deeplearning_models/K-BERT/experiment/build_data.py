...
def multi_task_build_dataset(dataset_smiles, labels_list, smiles_name, global_feature='ECFP4'):
    ...
    for i, smiles in enumerate(smilesList):
        ...
        if token_idx != 0:
            ...
            print('{} is transformed failed!'.format(smiles), flush=True)
            molecule_number = molecule_number - 1
            failed_molecule.append(smiles)
    print('{}({}) is transformed failed!'.format(failed_molecule, len(failed_molecule)), flush=True)
    return dataset
...
def load_data_for_splited(data_name=None, data_path='example.npy',task_type='classification',scaler=None):
    ...
    if task_type=='regression':
        ...
        if scaler=='RobustScaler':
            pt = RobustScaler()
            pt.fit_transform(labels.reshape(-1, 1))            
            print(f"RobustScaler center: {pt.center_}")
            print(f"RobustScaler scale (IQR): {pt.scale_}")
            with open(f'scalers/{data_name}_RobustScaler.pkl', 'wb') as f:
                pickle.dump(pt, f)
        if scaler=='StandardScaler':
            pt = StandardScaler()
            pt.fit_transform(labels.reshape(-1, 1))
            print(f"StandardScaler mean: {pt.mean_}")
            print(f"StandardScaler std: {pt.scale_}")
            with open(f'scalers/{data_name}_StandardScaler.pkl', 'wb') as f:
                pickle.dump(pt, f)
        if scaler=='PowerTransformer':
            pt = PowerTransformer(method='yeo-johnson', standardize=True)
            labels = pt.fit_transform(labels.reshape(-1, 1))
            print(f"PowerTransformer lambdas: {pt.lambdas_}")
            with open(f'scalers/{data_name}_PowerTransformer.pkl', 'wb') as f:
                pickle.dump(pt, f)
    else:
        mean = None
        std = None
    print('Training set: {}; Validation set: {}; Test set: {}'.format(len(train_set), len(val_set), len(test_set)))
...
def load_data_for_random_splited(data_path='example.npy', shuffle=True):
    ...
    print('Training set: {}, Validation set: {}, Test set: {}, task number: {}'.format(
            len(train_set), len(val_set), len(test_set), task_number), flush=True)
    return train_set, val_set, test_set, task_number
