from sklearn.metrics import roc_auc_score, mean_absolute_error, precision_recall_curve, auc, r2_score
...
from experiment.build_data import standardization_np, re_standar_np
...
def run_a_train_global_epoch(args, epoch, model, data_loader, loss_criterion, optimizer, task_type):
    if task_type == 'classifcation':
        model.train()
        train_meter = Meter()
        for batch_id, batch_data in enumerate(data_loader):
            ...
            optimizer.step()
            print('epoch {:d}/{:d}, batch {:d}/{:d}, loss {:.4f}'.format(
                epoch + 1, args['num_epochs'], batch_id + 1, len(data_loader), loss.item()), flush=True)
        ...
        if args['scaler']:
            with open(f'scalers/{args["data_name"]}_{args["scaler"]}.pkl', 'rb') as f:
                scaler = pickle.load(f)  
        else:
            scaler=None          
        model.train()
        train_meter = Meter(scaler)
        for batch_id, batch_data in enumerate(data_loader):
            smiles, token_idx, global_labels, mask = batch_data
            if len(smiles)<2:
                continue
            ...
            train_meter.update(logits_global, global_labels, mask=mask)
            if scaler:
                global_labels = scaler.transform(global_labels.detach().cpu().numpy())
                global_labels = torch.from_numpy(global_labels).to(args['device'])
            loss = (loss_criterion(logits_global, global_labels).float()).mean()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def run_an_eval_global_epoch(args, model, data_loader, task_type, out_path=None):
    if task_type == 'classification':
        ...
        if out_path:
            result_pd = pd.DataFrame({'smiles': smiles_list, 'pred': y_pred_list, 'label': y_true_list})
            result_pd.to_csv(out_path + '_result.csv', index=False) 
        # save prediction
        ...
        prc = eval_meter.roc_precision_recall_score()[0]
        ...
        pre = pre[1]
        err = 1 - accuracy
        result = [auc, prc, accuracy, se, sp, f1, pre, rec, err, mcc]
        return result
    else:
        model.eval()
        if args['scaler']:
        ...
        if out_path:
            result_pd = pd.DataFrame({'smiles': smiles_list, 'pred': y_pred_list, 'label': y_true_list})   
            result_pd.to_csv(out_path + '_result.csv', index=False)     
        rmse = eval_meter.compute_metric('rmse')[0]
        r2 = eval_meter.compute_metric('r2')[0]
        mae = eval_meter.compute_metric('mae')[0]
        result = [r2, rmse, mae]
        return result
...
class EarlyStopping(object):
    def __init__(self, pretrained_model='Null_early_stop.pth',
                 pretrain_layer=6, mode='higher', patience=10, task_name="None", savecheckpoint=True):
