...
from sklearn.metrics import roc_auc_score, accuracy_score, precision_recall_curve, r2_score, auc
...
def calc_acc_score(labels, preds, valid):
    """compute ACC and averaged across tasks"""
    if labels.ndim == 1:
        labels = labels.reshape(-1, 1)
        preds = preds.reshape(-1, 1)
    ...
    for i in range(labels.shape[1]):
        ...
        #AUC is only defined when there is at least one positive data.
        if len(np.unique(c_label)) == 2:
            acc_list.append(accuracy_score(c_label, [1 if item>=0.5 else 0 for item in c_pred]))

    # print('Valid ratio: %s' % (np.mean(valid)))
    # print('Task evaluated: %s/%s' % (len(acc_list), labels.shape[1]))
    if len(acc_list) == 0:
        raise RuntimeError("No positively labeled data available. Cannot compute ACC.")

    return sum(acc_list)/len(acc_list)

def calc_rocprc_score(labels, preds, valid):
    """compute PRC-AUC and averaged across tasks"""
    if labels.ndim == 1:
        labels = labels.reshape(-1, 1)
        preds = preds.reshape(-1, 1)
    ...
    for i in range(labels.shape[1]):
        ...
        #AUC is only defined when there is at least one positive data.
        if len(np.unique(c_label)) == 2:
            precision, recall, _ = precision_recall_curve(c_label, c_pred)
            rocprc_list.append(auc(recall, precision))

    # print('Valid ratio: %s' % (np.mean(valid)))
    # print('Task evaluated: %s/%s' % (len(rocprc_list), labels.shape[1]))
    if len(rocprc_list) == 0:
        raise RuntimeError("No positively labeled data available. Cannot compute PRC-AUC.")

    return sum(rocprc_list)/len(rocprc_list)


def calc_r2(labels, preds):
    """tbd"""
