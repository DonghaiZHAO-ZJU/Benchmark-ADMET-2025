...
from src.utils import calc_r2, calc_rmse, calc_mae, exempt_parameters
...
def train(
    ...
    list_loss = []
    model.train()
    for atom_bond_graphs, bond_angle_graphs, _, labels in data_gen:
...
def evaluate(
        args, 
        model, label_mean, label_std,
        test_dataset, collate_fn, out_path=None):
    """
    ...
    model.eval()
    for atom_bond_graphs, bond_angle_graphs, smiles, labels in data_gen:
        atom_bond_graphs = atom_bond_graphs.tensor()
        bond_angle_graphs = bond_angle_graphs.tensor()
    ...
    return calc_r2(total_label, total_pred), calc_rmse(total_label, total_pred), calc_mae(total_label, total_pred)

def get_label_stat(dataset):
    """tbd"""
    labels = np.array([data['label'] for data in dataset])
    return np.mean(labels), np.std(labels), np.min(labels), np.max(labels)
...
def main(args, seed=2024):
    ...
    print('Train mean/std/min/max %s/%s/%s/%s' % get_label_stat(train_dataset))
    print('Valid mean/std/min/max %s/%s/%s/%s' % get_label_stat(valid_dataset))
    print('Test mean/std/min/max %s/%s/%s/%s' % get_label_stat(test_dataset))

    ### start train
    label_mean, label_std, _, _ = get_label_stat(train_dataset)
    ...
    for epoch_id in range(args.max_epoch):
        ...
        train_metric = evaluate(
                args, model, label_mean, label_std, 
                train_dataset, collate_fn)[0]
        val_metric = evaluate(
                args, model, label_mean, label_std, 
                valid_dataset, collate_fn)[0]
        test_metric = evaluate(
                args, model, label_mean, label_std, 
                test_dataset, collate_fn)[0]
        print(f'epoch: {epoch_id+1}/{args.max_epoch}, train_loss: {train_loss}, train r2: {train_metric}, valid r2: {val_metric}, test r2: {test_metric}')
    ...
    valid_result = evaluate(
            args, model, label_mean, label_std, 
            valid_dataset, collate_fn)
    test_result = evaluate(
            args, model, label_mean, label_std, 
            test_dataset, collate_fn, out_path=os.path.join(args.output_dir, f'{args.data_name}_{int((seed-2024)/10+1)}_test_result.csv'))
...
if __name__ == '__main__':
    ...
    if args.task == 'data':
        ...
        for i in range(5):
            seed = 2024+10*i
            train_result, valid_result, test_result = main(args, seed)
        ...
        result_pd.to_csv('./result_extra/GEM_' + args.data_name + '_all_result.csv', index=False)    
