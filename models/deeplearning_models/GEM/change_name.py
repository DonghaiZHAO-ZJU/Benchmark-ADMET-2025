...
for task in ['CHEMBL1862_Ki', 'CHEMBL1871_Ki', 'CHEMBL2034_Ki', 'CHEMBL2047_EC50', 
    ...
    origin_data = pd.read_csv(f'./data/raw_data/{task}.csv')
    ...
    data = np.load(f"features/gem_features_{task}.npz")
    ...
    df.to_csv(f"features/{task}_GEM_Embedding.csv", index=False)