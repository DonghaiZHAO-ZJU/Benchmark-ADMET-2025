...
def atom_to_graph(smiles,encoder_atom,encoder_bond):
    ...
    # 移除700原子的限制，设置更大的合理上限以防止内存问题
    Num_toms = len(tmp)
    if Num_toms > 10000:
        print(f"Warning: Molecule too large ({Num_toms} atoms), skipping: {smiles}")
        g = False
        ...
        if mmff_force_field(mol) == True:
            ...
            if num_conformers > 0:
                ...
                print(f"Warning: No conformers generated, skipping: {smiles}")
                g = False
        else:
            print(f"Warning: MMFF force field calculation failed, skipping: {smiles}")
            g = False
    return g
...
def get_dataset_cache_path(dataset_name, cache_dir='data/processed_data/graph_cache'):
    """根据数据集名称生成缓存文件路径"""
    # 创建缓存目录
    os.makedirs(cache_dir, exist_ok=True)

    # 使用数据集名称作为文件名
    filename = f"{dataset_name}_graphs.pth"
...
def load_dataset_cache(dataset_name):
    ...
    if os.path.exists(cache_path):
        try:
            ...
            _dataset_cache[dataset_name] = cache_data
            return cache_data
        except:
            # 如果缓存文件损坏，则返回空字典
            pass
...
def save_dataset_cache(dataset_name):
    """保存数据集的分子图缓存"""
    if dataset_name in _dataset_cache:
        try:
            cache_path = get_dataset_cache_path(dataset_name)
            torch.save(_dataset_cache[dataset_name], cache_path)
        except:
            # 如果保存失败，则忽略
            pass


def get_graph_cache_path(smiles, cache_dir='data/processed_data/graph_cache'):
    """根据SMILES字符串生成缓存文件路径"""
    # 创建缓存目录
    os.makedirs(cache_dir, exist_ok=True)

    # 使用SMILES字符串的哈希值作为文件名
    hash_object = hashlib.md5(smiles.encode())
    filename = hash_object.hexdigest() + '.pth'
    return os.path.join(cache_dir, filename)


def path_complex_mol(Smile, encoder_atom, encoder_bond, use_cache=True, dataset_name=None):
    # 如果提供了数据集名称，使用数据集级别的缓存
    if use_cache and dataset_name:
        # 加载数据集缓存
        dataset_cache = load_dataset_cache(dataset_name)

        # 如果分子图已在缓存中，则直接返回
        if Smile in dataset_cache:
            return dataset_cache[Smile]
        ...
        # 如果计算成功，则保存到数据集缓存
        if g != False:
            dataset_cache[Smile] = g
            _dataset_cache[dataset_name] = dataset_cache
            # 异步保存缓存（避免频繁IO）
            # save_dataset_cache(dataset_name)
            return g
        else:
            return False

    # 如果没有提供数据集名称，使用原有的单分子缓存机制
    elif use_cache:
        cache_path = get_graph_cache_path(Smile)
...
def save_all_dataset_caches():
    """保存所有数据集的缓存"""
    for dataset_name in _dataset_cache:
        save_dataset_cache(dataset_name)
