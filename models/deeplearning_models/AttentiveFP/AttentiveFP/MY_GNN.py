...
import datetime
from sklearn.metrics import accuracy_score, roc_auc_score, mean_squared_error, precision_recall_curve, auc, r2_score, mean_absolute_error
import torch
...
from AttentiveFP.build_dataset import standardization_np, re_standar_np
...
class Meter(object):
    ...
    def return_pred_true(self):
        ...
        if (self.mean is not None) and (self.std is not None):
            y_pred = re_standar_np(y_pred, self.mean, self.std)
        y_true = torch.cat(self.y_true, dim=0)
    ...
    def compute_metric(self, metric_name, reduction='mean'):
        ...
        """
        assert metric_name in ['roc_auc', 'l1', 'rmse', 'mae', 'roc_prc', 'r2', 'return_pred_true', ['accuracy', 'roc_auc', 'roc_prc'], ['rmse', 'r2', 'mae'], ['roc_auc', 'roc_prc', 'accuracy'], ['r2', 'mae', 'rmse']], \
        ...
        if metric_name == ['accuracy', 'roc_auc', 'roc_prc']:
            return [self.accuracy(), self.roc_auc_score(), self.roc_precision_recall_score()]
        if metric_name == ['rmse', 'r2', 'mae']:
            return [self.rmse(), self.r2(), self.mae()]
        if metric_name == ['roc_auc', 'roc_prc', 'accuracy']:
            return [self.roc_auc_score(), self.roc_precision_recall_score(), self.accuracy()]
        if metric_name == ['r2', 'mae', 'rmse']:
            return [self.r2(), self.mae(), self.rmse()]
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion_c, loss_criterion_r, optimizer, mean, std, task_weight=None):
    model.train()
    train_meter_c = Meter()
    train_meter_r = Meter(mean, std)
    total_loss = 0
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        if args['task_class'] == 'classification_regression':
            ...
            optimizer.zero_grad()
            loss.backward()
            total_loss = total_loss + len(smiles)*loss
            optimizer.step()
            train_meter_c.update(logits_c, labels_c, mask_c)
            train_meter_r.update(logits_r, labels_r, mask_r)
            del attentivefp_bg, mask, labels, attentivefp_node_feats, attentivefp_edge_feats, loss, logits_c, logits_r, labels_c, labels_r, mask_c, mask_r
            ...
            del attentivefp_bg, mask, labels, attentivefp_node_feats, attentivefp_edge_feats, loss, logits
            torch.cuda.empty_cache()
        else:
            train_meter_r.update(logits, labels, mask)
            # chose loss function according to task_weight
            labels = standardization_np(labels, mean, std)
            if task_weight is None:
            ...
            torch.cuda.empty_cache()
    if args['task_class'] == 'classification_regression':
        train_score = np.mean(train_meter_c.compute_metric(args['classification_metric_name'][0]) +
                              train_meter_r.compute_metric(args['regression_metric_name'][0]))
        print('epoch {:d}/{:d}, training {} {:.4f}'.format(
            epoch + 1, args['num_epochs'], 'r2+auc', train_score), flush=True)
        return train_score, total_loss / 999
    elif args['task_class'] == 'classification':
        train_score = np.mean(train_meter_c.compute_metric(args['classification_metric_name'][0]))
        print('epoch {:d}/{:d}, training {}: {:.4f}'.format(
            epoch + 1, args['num_epochs'], args['classification_metric_name'][0], train_score), flush=True)
        return train_score, total_loss / 999
    else:
        train_score = np.mean(train_meter_r.compute_metric(args['regression_metric_name'][0]))
        print('epoch {:d}/{:d}, training {}: {:.4f}'.format(
            epoch + 1, args['num_epochs'], args['regression_metric_name'][0], train_score), flush=True)
        return train_score, total_loss / 999
...
def run_an_eval_epoch_detail(args, model, data_loader, mean, std, out_path=None):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter(mean, std)
    smiles_list = []
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            smiles, attentivefp_bg, labels, mask = batch_data
            ...
            if args['task_class'] == 'classification_regression':
                ...
                del smiles, attentivefp_bg, mask, labels, attentivefp_node_feats, attentivefp_edge_feats, logits_c, logits_r, labels_c, labels_r, mask_c, mask_r
                torch.cuda.empty_cache()
            elif args['task_class'] == 'classification':
                # Mask non-existing labels
                eval_meter_c.update(logits, labels, mask)
                del smiles, attentivefp_bg, mask, labels, attentivefp_node_feats, attentivefp_edge_feats, logits
                torch.cuda.empty_cache()
            else:
                # Mask non-existing labels
                ...
                torch.cuda.empty_cache()                                    
        if args['task_class'] == 'classification_regression':
            list1 = eval_meter_c.compute_metric(args['classification_metric_name'])
            list2 = eval_meter_r.compute_metric(args['regression_metric_name'])
