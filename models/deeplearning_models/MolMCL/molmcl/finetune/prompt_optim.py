...
def get_probs(init_weights, temperature=0.5, act='softmax'):
    ...
    if act == 'neg-softmax':
        init_probs = 1 - init_probs*2
    return init_probs


def get_roughness_index(inputs):
    Y, X, metric = inputs
    return RoughnessIndex(Y=Y, X=X, metric=metric, verbose=False).compute_index()


def compute_ri(graph_reps, labels, init_weights, temperature=0.5, metric='euclidean', act='softmax'):
    assert len(labels.size()) == 2
    init_probs = get_probs(init_weights, temperature=temperature, act=act)
    ...
    for li in range(labels.size(1)):
        Y = np.array([e.item() for e in labels[:, li]])
        inputs = [(Y, np.array(graph_rep_ps[i].cpu()), metric) for i in range(graph_rep_ps.size(0))]
        ri_list = []
        pool = multiprocessing.Pool(8)
        for value in pool.imap(get_roughness_index, inputs):
            ri_list.append(value)

        ri_ps += np.array(ri_list) / labels.size(1)

        # ri_ps += np.array([RoughnessIndex(Y=np.array([e.item() for e in labels[:, li]]),
        #                                   X=np.array(graph_rep_ps[i].cpu()),
        #                                   metric=metric,
...
def generate_data(graph_reps, labels, num_channels=3, num_inits=10, temperature=0.5, metric='euclidean', act='softmax'):
    ...
    # Generate weight via normal distribution between (-1, 1):
    # init_weights = torch.normal(mean=0, std=1, size=(num_inits, num_channels)).to(graph_reps.device)

    # Generate weight via linspace between (-1, 1):
    init_weights = np.linspace(-1, 1, num=11)
    init_weights = torch.Tensor(list(itertools.product(init_weights, repeat=num_channels)))

    ri_ps = -compute_ri(graph_reps, labels, init_weights, temperature=temperature, metric=metric, act=act)

    sorted_inds = np.argsort(ri_ps, axis=0)
    init_probs = get_probs(init_weights, temperature=temperature, act=act)
    for i in sorted_inds[-10:]:
        print(init_probs[i], f'rogi={ri_ps[i].item()}')
...
def optimize_prompt_weight_ri(graph_reps, labels, n_runs, n_inits, n_points, n_restarts, n_samples, temperature, metric,
                              act='softmax', skip_bo=False, verbose=False):
    ...
    init_x, init_y, best_init_y = \
        generate_data(graph_reps, labels, num_channels=n_channels-1, num_inits=n_inits,
                      temperature=temperature, metric=metric, act=act)
    ...
    if verbose:
        ...
        print("Best prompt probs:", get_probs(init_x[torch.argmax(init_y)].unsqueeze(0),
                                              temperature=temperature, act=act))
