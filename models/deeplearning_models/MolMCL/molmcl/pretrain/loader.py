...
class MoleculeDataset(Dataset):
    def __init__(self, data_dir, num_candidates=5, feat_type='super_rich'):
        ...
        with open('/data/yuw253/precompute/data_shard_ids.pkl', 'rb') as f:
            self.data_shard_ids = pickle.load(f)
        self.keys = list(self.data_shard_ids.keys())
        # self.keys = [key for key in self.data_shard_ids if key[0] == 'z']
        
    def get_processed_data(self, key):
        lmdb_path = '/data/yuw253/precompute/{}.lmdb'.format(self.data_shard_ids[key])
