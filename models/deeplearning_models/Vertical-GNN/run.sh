  ...
  for split_method in "${split_methods[@]}"; do
    for task in "${all_tasks[@]}"; do
      ...
      if [[ " ${classification_tasks[@]} " =~ " ${task} " ]]; then
        python -u train.py --data_name ${task}_${split_method}_${seed} --task_type classification
      elif [[ " ${regression_tasks[@]} " =~ " ${task} " ]]; then
        python -u train.py --data_name ${task}_${split_method}_${seed} --task_type regression
      fi
    done
  done
done