...
def AttentiveFP_model(times, task_name,
    ...
    args['select_task_list'] = ["BBBP","hERG","Mutagenicity","oral_bioavailability","HLM_metabolic_stability", 
    ...
    os.makedirs('../result/{}_multitask/'.format(task_name), exist_ok=True)
    result_pd.to_csv('../result/{}_multitask/'.format(task_name) + args['task_name'] + '_result.csv', index=False)
    ...
    result_pd_val_average.to_csv('../result/{}_multitask/'.format(task_name)+args['task_name']+'_result_val_average.csv', index=None)
    result_pd_val_var.to_csv('../result/{}_multitask/'.format(task_name)+args['task_name']+'_result_val_var.csv', index=None)
    result_pd_test_average.to_csv('../result/{}_multitask/'.format(task_name)+args['task_name']+'_result_test_average.csv', index=None)
    result_pd_test_var.to_csv('../result/{}_multitask/'.format(task_name)+args['task_name']+'_result_test_var.csv', index=None)
...
if __name__ == '__main__':
    ...
    AttentiveFP_model(times=5, task_name=args.task_name)
