...
def config_logger(cfg, OUT_PATH="results/", time=False):
    ...
    writer_folder = os.path.join(OUT_PATH, cfg.version, data_name, 'SummaryWriter', config_string)
    if time:
    ...
    sys.stdout = open(f'logs_Perimeter/{data_name}/{cfg.dataset_name}.txt', 'w')
